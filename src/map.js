...
async function initMap(id) {
    await ymaps.ready(); // Используем актуальный метод ожидания загрузки API
    ...
    markersCollection.forEach((el, idx) => {
        ...
        const marker = new ymaps.Placemark(el.coordinate, { iconContent: content });
    ...
    if (isContactsListMap) {
        map.events.add('click', function (e) {
            ...
            if (target && target.properties && target.properties.get('iconContent')) {
                const officeName = target.properties.get('iconContent').dataset.office;
                const office = document.querySelector(`[data-office-name="${officeName}"]`);
                ...
                target.properties.get('iconContent').classList.add('_is-active');
